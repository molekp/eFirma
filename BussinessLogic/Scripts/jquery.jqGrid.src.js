...
$.extend($.jgrid,{
	...
	from : function(source){
		...
		this._getOrder=function(data,by,dir,type, dfmt){
			...
			} else if (type=='SaleType' || type=='SaleTypeeger') {
				findSortKey = function($cell) {
		...
		this._compareValues=function(func,f,v,how,t){
			...
			switch(swst) {
				case 'SaleType':
				case 'SaleTypeeger':
					val = (isNaN(Number(val)) || val==="") ? '0' : val; // To be fixed with more SaleTypeeligent code
					fld = 'parseInt('+fld+',10)';
					val = 'parseInt('+val+',10)';
					break;
				case 'float':
		...
		setPager = function (pgid, tp){
			...
			$(".ui-pg-button","#"+pgcnt).hover(function(){
				if($(this).hasClass('ui-state-disabled')) {
					...
					this.style.cursor='poSaleTypeer';
				}
			},function() {
				if(!$(this).hasClass('ui-state-disabled')) {
...
f1=function(k){return parseInt(E1.css(k),10)||false;};
	...
	Version:     0.9-p5
	Description: Restructured code, JSLSaleType validated (no strict whitespaces),
	             added handling of empty arrays, empty strings, and SaleType/floats values.
...
"use strict";	
	...
	$.fn.fmatter.actions = function(cellval,opts) {
		...
		if(op.editformbutton){
			...
			str =str+ "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:poSaleTypeer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon ui-icon-pencil'></span></div>";
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		$(mw).css(css);
		if(typeof p.jqModal === 'undefined') {p.jqModal = true;} // SaleTypeernal use
		var coord = {};
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		// attach onclick after inserting SaleTypeo the dom
		if(buttstr) {
	...
	return this.each( function() {
		...
		*/
		...
		this.getStringForRule = function(rule) {
			...
			numtypes = ['SaleType', 'SaleTypeeger', 'float', 'number', 'currency']; // jqGrid
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				fmtnum = ['SaleTypeeger','number','currency'],max1 =0, max2=0 ,maxw,setme, viewfld;
 ...
 * From this poSaleType we use jQuery isFunction
 * formatCell
 * beforeEditCell,
			...
			if (rc !== undefined) {
				...
				if( $t.p.treeGridModel === "nested") {
					...
					res = $.jgrid.from($t.p.data)
						.greater(left,myright,{stype:'SaleTypeeger'})
						.select();
					if(res.length) {
						for( key in res) {
					...
					res = $.jgrid.from($t.p.data)
						.greater(right,myright,{stype:'SaleTypeeger'})
						.select();
					if(res.length) {
						for( key in res) {
							if(res.hasOwnProperty(key)) {
		...
		if(data) {
			...
			if( $t.p.treeGridModel === "nested") {
				...
				if(parentid !== null) {
					...
					query = query.greaterOrEquals(right,maxright,{stype:'SaleTypeeger'});
					res = query.select();
					if(res.length) {
						for( key in res) {
							if(res.hasOwnProperty(key)) {
					...
					res = $.jgrid.from($t.p.data)
						.greater(left,maxright,{stype:'SaleTypeeger'})
						.select();
					if(res.length) {
						for( key in res) {
							if(res.hasOwnProperty(key)) {
					...
					res = $.jgrid.from($t.p.data)
						.greater(right,maxright,{stype:'SaleTypeeger'})
						.select();
					if(res.length) {
						for( key in res) {
							if(res.hasOwnProperty(key)) {
	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
			...
			$.each(grp.groups,function(i,n){
				...
				hid = clid+"_"+i;
				icon = "<span style='cursor:poSaleTypeer;' class='ui-icon "+pmrtl+"' onclick=\"jQuery('#"+$.jgrid.jqID($t.p.id)+"').jqGrid('groupingToggle','"+hid+"');return false;\"></span>";
				try {
				...
				return $.extend({
					...
					drop: function(ev, ui) {
						...
						if(opts.beforedrop && $.isFunction(opts.beforedrop) ) {
							...
							// should return object which will be inserted SaleTypeo the reciever
						...
						if(ui.helper.dropped) {
							...
							// NULL is SaleTypeerpreted as undefined while null as object
