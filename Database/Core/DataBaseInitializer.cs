    ...
    public class DataBaseInitializer : DropCreateDatabaseAlways<DataBaseContext>
    {
        ...
        private List<ProductItem> CreateProducts(List<ProductType> a_itemtypes, List<SaleType> a_saleTypes)
        {
            return new List<ProductItem>{
                ...
                new ProductItem{ IdItem = 2, Name = "Ser zółty gouda", Price = new decimal(17.90), Quantity = 10, Vin = "6573683758", ItemState = ItemState.InWarehouse,ExpirationTime =DateTime.Now,ItemType = a_itemtypes[0],SaleType = a_saleTypes[0]},
        ...
        private List<EStore> CreateEStores(List<Warehouse> warehouses, List<EStoreShipmentType> a_shipmentTypes)
        {
            return new List<EStore>
                {
                        new EStore() { Name = "eSklep spożywczy", UniqHash =  Convert.ToBase64String(Encoding.UTF8.GetBytes(DateTime.Now.ToString())) , Warehouses = warehouses, EStoreShipmentType = a_shipmentTypes},
                        new EStore() { Name = "eSklep odzież",  UniqHash =  Convert.ToBase64String(Encoding.UTF8.GetBytes(DateTime.Now.ToString())), Warehouses = new List<Warehouse>(), EStoreShipmentType = new List<EStoreShipmentType>()}
                };
        }
        ...
        private List<Currency> CreateCurrencies()
        {
            return new List<Currency>
                {
                        new Currency {Name = "Polski złoty", Code = "PLN"},
                        new Currency {Name = "Dolar amerykański", Code = "USD"},
